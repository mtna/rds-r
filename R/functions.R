...
select <- function(url, catalog, dataProduct, inject=FALSE, autoPage=TRUE, classLimit=NULL, classOffset=NULL, codeEquals=NULL, codeLimit=NULL, codeOffset=NULL,
  ...
  info <- dataSet@info
  columnInfo <- info
  columnCount <- columnInfo$columnCount[[1]]
  ...
  if(autoPage){
    ...
    while(columnInfo$moreColumns[[1]]){
      Sys.sleep(sleep)
  ...
  if(autoPage){
    while(rowInfo$moreRows[[1]]){
      ...
      if(i == 1){
        ...
        myRowLimit <- rowInfo$rowLimit
        offset <- offset[[1]]+rowInfo$rowLimit[[1]]
      }
...
selectPage <- function(url,catalog,dataProduct,inject=FALSE,cols=NULL,where=NULL,orderby=NULL,
  ...
  if(!is.null(json$variables)){
    #crreate a data frame from the returned list
