...
The first thing we need to do is set up a connection to the RDS server and get the data product we want to use, in this case we will use a subset of the Canadian Labor Force Survey.

```{r }
# provide the root application URL to get the server
rds <- get.rds("https://covid19.richdataservices.com/rds")
catalog <- rds.r::getCatalog(rds, "ca")
dataProduct <- rds.r::getDataProduct(catalog, "pums_lfs")
```
...
tabulation <- rds.tabulate(dataProduct, dimensions = "prov")

ggplot(tabulation@records, aes(x = PROV, y = count, color = PROV)) +
...
```{r fig.width=7, fig.height=4}
tabulation <-
  rds.tabulate(dataProduct, dimensions = "prov", inject = T)
...
ggplot(tabulation@records, aes(x = PROV, y = percent, color = PROV)) +
  geom_bar(stat = "identity", fill = "white") + 
...
Multiple dimensions and measures can be used if desired. In this example we will get a cross tabulation of province, marriage status, and sex, with their percentages returned to us. 

```{r fig.width=7, fig.height=4}
tabulation <-
  rds.tabulate(
    dataProduct,
    dimensions = c("prov", "marstat", "sex"),
...
# Make the plot
ggplot(tabulation@records, aes(x=PROV, y=percent, fill=MARSTAT)) + geom_bar(stat="identity") +
